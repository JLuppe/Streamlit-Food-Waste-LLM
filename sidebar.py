...
def load_sidebar():
    try:
        st.session_state["API_KEY"] = st.sidebar.text_input("User Gemini API Key", type='password')

        # FOUNDATIONAL FILES
        title_column = st.sidebar.columns([1, 3, 1])
        st.sidebar.title("Foundational Knowledge", width="stretch")
        ...
        for file in filestr_in_data:
            # context_dropdown.button("View", key=file + "_button", on_click=set_pdf_binary, args=(file,))
        ...
        if st.session_state["sidebar_uploaded_files"]:
            st.session_state["uploaded_files_embeddings"] = convert_docs_to_dict_chunks(st.session_state["sidebar_uploaded_files"])
            # st.info(f"st.session_state:['sidebar_uploaded_files']: {st.session_state["sidebar_uploaded_files"]}")
            generate_uploaded_file_buttons()

        # RESET CHAT HISTORY
        st.sidebar.button("Reset Chat History", on_click = reset_conversation)
    except Exception as e:
        st.info("Error in load_sidebar in sidebar.py: {e}")
...
def generate_uploaded_file_buttons():
    try:
        ...
        for file in uploadedFiles:
            uf_col1.checkbox(label= file.name, key=file.name, on_change=include_in_context, args=(file.name,))
            uf_col2.button("View", key=file.name + "_button", on_click=set_viewed_file_uploaded, args=(file,))
    except Exception as e:
        st.info("Error in generate_uploaded_file_buttons in sidebar.py: {e}")
